namespace OAuthCSharp
{
    ...
    public static class OAuthUtils
    {
        ...
        private static string GenerateNonce(string timestamp)
        {
            ...
            {
                Key = Encoding.ASCII.GetBytes(Encoding.ASCII.GetString(buffer))
            };
        ...
        private static string GenerateSignatureBaseString(string url, string httpMethod, List<ProtocolParameter> protocolParameters)
        {
            ...
            switch (signatureMethod)
            {
                case SignatureMethod.HMACSHA1:
                    var hmacsha1 = new HMACSHA1
                    {
                        ...
                        UrlEncode(consumerSecret) + "&" + UrlEncode(tokenSecret ?? string.Empty))
                    };
        ...
        public static AuthorizeHeader GetRequestTokenAuthorizationHeader(string url, string realm, string consumerKey, string consumerSecret, string callbackUrl, SignatureMethod signatureMethod = SignatureMethod.HMACSHA1, string httpMethod = "POST")
        {
        ...
        public static AuthorizeHeader GetAccessTokenAuthorizationHeader(string url, string realm, string consumerKey, string consumerSecret, string token, string verifier, string tokenSecret, SignatureMethod signatureMethod = SignatureMethod.HMACSHA1, string httpMethod = "POST")
        {
            var timestamp = GenerateTimeStamp();
            var nounce = GenerateNonce(timestamp);
        ...
        public static AuthorizeHeader GetUserInfoAuthorizationHeader(string url, string realm, string consumerKey, string consumerSecret, string token, string tokenSecret, SignatureMethod signatureMethod = SignatureMethod.HMACSHA1, string httpMethod = "POST")
        {
